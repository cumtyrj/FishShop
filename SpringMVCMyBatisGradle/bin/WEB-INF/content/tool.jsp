					...
					<div class="row">
						...
						<table border="1">
							...
							<c:forEach items="${requestScope.tool_list }" var="tool">
								...
								<td>￥${tool.price }</td>
								<td>${tool.detail }</td>
								<td><img src="source/img/${tool.image}" height="60"></td>
								<td><a href="save?good_id=${tool.good_id}"><button type="button" class="btn btn-default" >加入购物车</button></a>
								<td><a class="btn btn-danger" href="savecollect?good_id=${tool.good_id}" role="button">收藏</a></td>			
								<td>${tool.sales }</td>
								<td>${tool.inventory }</td>
								</tr>
							</c:forEach>
							</table>
						</div>
