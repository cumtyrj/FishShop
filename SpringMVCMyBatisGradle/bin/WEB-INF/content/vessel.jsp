					...
					<div class="row">
						...
						<table border="1">
							...
							<c:forEach items="${requestScope.vessel_list }" var="vessel">
								...
								<td>￥${vessel.price }</td>
								<td>${vessel.detail }</td>
								<td><img src="source/img/${vessel.image}" height="60"></td>
								<td><a href="save?good_id=${vessel.good_id}"><button type="button" class="btn btn-default" >加入购物车</button></a>
								<td><a class="btn btn-danger" href="savecollect?good_id=${vessel.good_id}" role="button">收藏</a></td>
							
								<td>${vessel.sales }</td>
								<td>${vessel.inventory }</td>
								</tr>
							</c:forEach>
							</table>
						</div>
