					...
					<div class="row">
						...
						<table border="1">
							...
							<c:forEach items="${requestScope.chair_list }" var="chair">
								...
								<td>￥${chair.price }</td>
								<td>${chair.detail }</td>
								<td><img src="source/img/${chair.image}" height="60"></td>
								<td><a href="save?good_id=${chair.good_id}"><button type="button" class="btn btn-default" >加入购物车</button></a>
								<td><a class="btn btn-danger" href="savecollect?good_id=${chair.good_id}" role="button">收藏</a></td>
								<td>${chair.sales }</td>
								<td>${chair.inventory }</td>
								</tr>
							</c:forEach>
							</table>
						</div>
